...
import { liquidacoes } from "@/lib/db/schema";
...
export async function GET() {
  try {
    const db = getDb();
    const rows = await db.select().from(liquidacoes);
    return NextResponse.json(rows.map(rowToFase));
  } catch (e) {
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Erro ao buscar liquidações" },
      { status: 500 }
    );
  }
}
  ...
  try {
    ...
    if (mode === "replace") {
      ...
      if (exercicios.length) {
        await db
          .delete(liquidacoes)
          .where(inArray(sql`split_part(${liquidacoes.numeroEmpenho}, '/', 2)`, exercicios));
      }
    ...
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Erro ao salvar liquidações" },
      { status: 500 }
    );
  }
}
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Erro ao limpar liquidações" },
      { status: 500 }
    );
  }
}
...
async function batchInsert(
  ...
  for (let i = 0; i < inserts.length; i += CHUNK) {
    await db.insert(liquidacoes).values(inserts.slice(i, i + CHUNK));
  }
}
